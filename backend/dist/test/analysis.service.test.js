...
describe('Analysis Service', () => {
    ...
    describe('analyzeStatements', () => {
        ...
        it('should throw error if client not found', async () => {
            ...
            await expect(analysisService.analyzeStatements({
                statementFileIds: ['file-1'],
        ...
        it('should throw error if statement files not parsed', async () => {
            ...
            await expect(analysisService.analyzeStatements({
                statementFileIds: ['file-1'],
                clientId: 'client-1'
            })).rejects.toThrow(new ApiError(httpStatus.BAD_REQUEST, 'All statement files must be successfully parsed before analysis'));
        });
    ...
    describe('getAnalysisTransactions', () => {
        ...
        it('should throw error if analysis not found', async () => {
            vi.mocked(prisma.analysis.findUnique).mockResolvedValue(null);
            await expect(analysisService.getAnalysisTransactions('invalid-analysis')).rejects.toThrow(new ApiError(httpStatus.NOT_FOUND, 'Analysis not found'));
        });
    ...
    describe('deleteAnalysisById', () => {
        ...
        it('should throw error if analysis not found', async () => {
            vi.mocked(prisma.analysis.findUnique).mockResolvedValue(null);
            await expect(analysisService.deleteAnalysisById('invalid-analysis')).rejects.toThrow(new ApiError(httpStatus.NOT_FOUND, 'Analysis not found'));
        });
    });
});
