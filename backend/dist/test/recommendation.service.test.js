...
describe('Recommendation Service', () => {
    ...
    describe('generateRecommendations', () => {
        ...
        it('should throw error if analysis not completed', async () => {
            ...
            const criteria = { analysisId: 'analysis-1' };
            await expect(recommendationService.generateRecommendations(criteria)).rejects.toThrow(ApiError);
            await expect(recommendationService.generateRecommendations(criteria)).rejects.toThrow('Analysis must be completed');
        });
    ...
    describe('getRecommendations', () => {
        ...
        it('should apply sorting options', async () => {
            mockPrisma.recommendation.findMany.mockResolvedValue(mockRecommendations);
            const filter = {};
            const options = { sortBy: 'priority', sortType: 'asc' };
            await recommendationService.getRecommendations(filter, options);
            expect(mockPrisma.recommendation.findMany).toHaveBeenCalledWith({
                where: filter,
                select: expect.any(Object),
    ...
    describe('approveRecommendation', () => {
        ...
        it('should throw error if recommendation not found', async () => {
            mockPrisma.recommendation.findUnique.mockResolvedValue(null);
            await expect(recommendationService.approveRecommendation('nonexistent', 'admin@test.com')).rejects.toThrow(ApiError);
            await expect(recommendationService.approveRecommendation('nonexistent', 'admin@test.com')).rejects.toThrow('Recommendation not found');
        });
        it('should throw error if recommendation not pending', async () => {
            ...
            await expect(recommendationService.approveRecommendation('rec-1', 'admin@test.com')).rejects.toThrow(ApiError);
            await expect(recommendationService.approveRecommendation('rec-1', 'admin@test.com')).rejects.toThrow('Only pending recommendations can be approved');
        });
    });
    describe('rejectRecommendation', () => {
        ...
        it('should throw error if recommendation not found', async () => {
            mockPrisma.recommendation.findUnique.mockResolvedValue(null);
            await expect(recommendationService.rejectRecommendation('nonexistent', 'admin@test.com')).rejects.toThrow(ApiError);
            await expect(recommendationService.rejectRecommendation('nonexistent', 'admin@test.com')).rejects.toThrow('Recommendation not found');
        });
        it('should throw error if recommendation not pending', async () => {
            ...
            await expect(recommendationService.rejectRecommendation('rec-1', 'admin@test.com')).rejects.toThrow(ApiError);
            await expect(recommendationService.rejectRecommendation('rec-1', 'admin@test.com')).rejects.toThrow('Only pending recommendations can be rejected');
        });
    });
});
