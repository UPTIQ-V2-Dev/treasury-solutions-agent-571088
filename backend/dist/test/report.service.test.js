...
import * as storageMain from "../storage/main.js";
...
describe('Report Service', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        // Setup storage mock
        storageMain.getInstance.mockReturnValue(mockStorageInstance);
    ...
    describe('generateReport', () => {
        it('should generate a PDF report successfully', async () => {
            ...
            prisma.report.create.mockResolvedValue({ ...mockReport, status: 'generating' });
        ...
        it('should generate an Excel report successfully', async () => {
            ...
            prisma.analysis.findUnique.mockResolvedValue(mockAnalysis);
            prisma.report.create.mockResolvedValue({ ...mockReport, status: 'generating' });
            prisma.report.update.mockResolvedValue({ ...mockReport, format: 'EXCEL' });
            mockStorageInstance.uploadData.mockResolvedValue(undefined);
            const reportData = {
                analysisId: 'analysis123',
        ...
        it('should throw error when analysis not found', async () => {
            ...
            await expect(reportService.generateReport(reportData)).rejects.toThrow(new ApiError(httpStatus.NOT_FOUND, 'Analysis not found'));
        });
        it('should throw error for unsupported format', async () => {
            ...
            await expect(reportService.generateReport(reportData)).rejects.toThrow(new ApiError(httpStatus.BAD_REQUEST, 'Unsupported report format'));
        });
    ...
    describe('getReportDownloadStream', () => {
        ...
        it('should return download stream successfully', async () => {
            prisma.report.findUnique.mockResolvedValue(mockReport);
            prisma.report.update.mockResolvedValue({ ...mockReport, downloadCount: 1 });
        ...
        it('should throw error when report not found', async () => {
            prisma.report.findUnique.mockResolvedValue(null);
            await expect(reportService.getReportDownloadStream('nonexistent')).rejects.toThrow(new ApiError(httpStatus.NOT_FOUND, 'Report file not found'));
        });
        it('should throw error when file path is null', async () => {
            prisma.report.findUnique.mockResolvedValue({ ...mockReport, filePath: null });
            await expect(reportService.getReportDownloadStream('report123')).rejects.toThrow(new ApiError(httpStatus.NOT_FOUND, 'Report file not found'));
        });
        it('should throw error when storage access fails', async () => {
            ...
            await expect(reportService.getReportDownloadStream('report123')).rejects.toThrow(new ApiError(httpStatus.NOT_FOUND, 'Report file could not be accessed from storage'));
        });
    });
    ...
    describe('deleteReportById', () => {
        ...
        it('should throw error when report not found', async () => {
            prisma.report.findUnique.mockResolvedValue(null);
            await expect(reportService.deleteReportById('nonexistent')).rejects.toThrow(new ApiError(httpStatus.NOT_FOUND, 'Report not found'));
        });
