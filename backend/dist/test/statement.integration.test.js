...
describe('Statement Integration Tests', () => {
    ...
    describe('Statement File Upload', () => {
        ...
        test('should return error for non-existent client', async () => {
            ...
            await expect(statementController.uploadStatement(mockReq, mockRes, mockNext)).rejects.toThrow(expect.objectContaining({ statusCode: httpStatus.NOT_FOUND }));
        });
    });
    describe('Upload Status', () => {
        ...
        test('should return error for non-existent upload', async () => {
            ...
            await expect(statementController.getUploadStatus(mockReq, mockRes, mockNext)).rejects.toThrow(expect.objectContaining({ statusCode: httpStatus.NOT_FOUND }));
        });
    });
...
2024-01-16,Deposit,200.00,1200.00`;
        ...
        test('should return error for non-existent statement file', async () => {
            ...
            prisma.statementFile.findUnique.mockResolvedValue(null);
            await expect(statementController.parseStatement(mockReq, mockRes, mockNext)).rejects.toThrow(expect.objectContaining({ statusCode: httpStatus.NOT_FOUND }));
        });
    });
