...
vi.mocked(prisma).token = mockPrisma.token;
vi.mocked(prisma).user = mockPrisma.user;
describe('Auth Service', () => {
    ...
    describe('loginUserWithEmailAndPassword', () => {
        ...
        it('should throw error for invalid credentials', async () => {
            ...
            await expect(authService.loginUserWithEmailAndPassword('john@example.com', 'wrongpassword')).rejects.toThrow(new ApiError(httpStatus.UNAUTHORIZED, 'Incorrect email or password'));
        });
        it('should throw error for non-existent user', async () => {
            vi.mocked(userService.getUserByEmail).mockResolvedValue(null);
            await expect(authService.loginUserWithEmailAndPassword('nonexistent@example.com', 'password123')).rejects.toThrow(new ApiError(httpStatus.UNAUTHORIZED, 'Incorrect email or password'));
        });
    });
    describe('logout', () => {
        ...
        it('should throw error for invalid token', async () => {
            ...
            await expect(authService.logout('invalid-token')).rejects.toThrow(new ApiError(httpStatus.NOT_FOUND, 'Not found'));
        });
    });
    describe('refreshAuth', () => {
        it('should return new tokens on valid refresh token', async () => {
            ...
            vi.mocked(tokenService.verifyToken).mockResolvedValue(mockTokenData);
        ...
        it('should throw error on invalid refresh token', async () => {
            ...
            await expect(authService.refreshAuth('invalid-token')).rejects.toThrow(new ApiError(httpStatus.UNAUTHORIZED, 'Invalid or expired refresh token'));
        });
    });
    describe('resetPassword', () => {
        ...
        it('should throw error for invalid token', async () => {
            vi.mocked(tokenService.verifyToken).mockRejectedValue(new Error('Invalid token'));
            await expect(authService.resetPassword('invalid-token', 'newPassword123')).rejects.toThrow(new ApiError(httpStatus.UNAUTHORIZED, 'Invalid or expired reset token'));
        });
        it('should throw error if user not found', async () => {
            const mockTokenData = { userId: 123 };
            vi.mocked(tokenService.verifyToken).mockResolvedValue(mockTokenData);
            vi.mocked(userService.getUserById).mockResolvedValue(null);
            await expect(authService.resetPassword('valid-token', 'newPassword123')).rejects.toThrow(new ApiError(httpStatus.UNAUTHORIZED, 'Invalid or expired reset token'));
        });
    });
    describe('verifyEmail', () => {
        ...
        it('should throw error for invalid token', async () => {
            vi.mocked(tokenService.verifyToken).mockRejectedValue(new Error('Invalid token'));
            await expect(authService.verifyEmail('invalid-token')).rejects.toThrow(new ApiError(httpStatus.UNAUTHORIZED, 'Invalid or expired verification token'));
        });
    });
});
