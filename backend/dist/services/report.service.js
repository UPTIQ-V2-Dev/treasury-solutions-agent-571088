...
const generateReport = async (reportData) => {
    const { analysisId, templateId = 'comprehensive-treasury-report', format = 'PDF', title, sections, createdBy } = reportData;
    ...
    try {
        ...
        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, `Failed to generate report: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
};
/**
 * Generate PDF report
 */
const generatePDFReport = async (analysis, template, sections, title) => {
 ...
 */
const generateExcelReport = async (analysis, template, sections, title) => {
 ...
 */
const generateHTMLContent = async (analysis, template, sections, title) => {
    let html = `
    <!DOCTYPE html>
    <html>
    <head>
 ...
 */
const generateHTMLSection = async (analysis, section) => {
 ...
 */
const generateExecutiveSummaryHTML = (analysis) => {
 ...
 */
const generateAnalysisSummaryHTML = (analysis) => {
    const summary = analysis.summary;
    return `
        <div class="metrics-grid">
            <div class="metric-card">
 ...
 */
const generateLiquidityMetricsHTML = (analysis) => {
    const metrics = analysis.liquidityMetrics;
    const idleAnalysis = analysis.idleBalanceAnalysis;
    return `
        <div class="metrics-grid">
            <div class="metric-card">
 ...
 */
const generateSpendingBreakdownHTML = (analysis) => {
 ...
 */
const generateRecommendationsHTML = (analysis, config) => {
 ...
 */
const generateDataTablesHTML = async (analysis) => {
 ...
 * Add Excel section
 */
const addExcelSection = async (workbook, analysis, section) => {
 ...
 */
const addAnalysisSummarySheet = async (worksheet, analysis) => {
    const summary = analysis.summary;
 ...
 */
const addLiquidityMetricsSheet = async (worksheet, analysis) => {
    const metrics = analysis.liquidityMetrics;
    const idleAnalysis = analysis.idleBalanceAnalysis;
 ...
 */
const addSpendingBreakdownSheet = async (worksheet, analysis) => {
    const breakdown = analysis.spendingBreakdown;
 ...
 */
const addRecommendationsSheet = async (worksheet, analysis) => {
    const recommendations = analysis.Recommendation || [];
 ...
 */
const addDataTablesSheet = async (worksheet, analysis) => {
 ...
 */
const getReportDownloadStream = async (reportId) => {
 ...
 * Query reports
 */
const queryReports = async (filter, options, keys = [
    'id',
    'title',
    'format',
    'status',
    ...
    if (filter.analysisId)
        where.analysisId = filter.analysisId;
    if (filter.format)
...
const deleteReportById = async (reportId) => {
    ...
    if (report.filePath) {
        try {
            ...
            console.warn(`Failed to delete report file from storage: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
...
const formatDateRange = (analysis) => {
    const summary = analysis.summary;
...
const formatBenefitProjection = (projection) => {
