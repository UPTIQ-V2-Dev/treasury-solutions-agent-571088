 ...
 * @returns {Promise<object>}
 */
const updateConfig = async (configUpdates, updatedBy) => {
 ...
 * @returns {Promise<object>}
 */
const resetConfig = async (updatedBy) => {
    ...
    for (const [key, value] of Object.entries(config)) {
        ...
        if (key.includes('timeout') || key.includes('Timeout')) {
            if (typeof value !== 'number' || value < 1 || value > 1440) {
                // Max 24 hours
                throw new ApiError(httpStatus.BAD_REQUEST, `Invalid timeout value for ${key}: must be between 1 and 1440 minutes`);
            }
        }
        ...
        if (key.includes('expiry') || key.includes('Expiry')) {
            if (typeof value !== 'number' || value < 1 || value > 365) {
                // Max 1 year
                throw new ApiError(httpStatus.BAD_REQUEST, `Invalid expiry value for ${key}: must be between 1 and 365 days`);
            }
        }
        ...
        if (key.includes('retention') || key.includes('Retention')) {
            if (typeof value !== 'number' || value < 30 || value > 2555) {
                ...
                throw new ApiError(httpStatus.BAD_REQUEST, `Invalid retention value for ${key}: must be between 30 and 2555 days`);
            }
        }
        // Validate rate limits
        if (key.includes('rateLimit') || key.includes('RateLimit')) {
            if (typeof value !== 'number' || value < 1 || value > 10000) {
                throw new ApiError(httpStatus.BAD_REQUEST, `Invalid rate limit value for ${key}: must be between 1 and 10000`);
            }
        }
        // Validate file sizes (in MB)
        if (key.includes('fileSize') || key.includes('FileSize')) {
            if (typeof value !== 'number' || value < 1 || value > 100) {
                throw new ApiError(httpStatus.BAD_REQUEST, `Invalid file size value for ${key}: must be between 1 and 100 MB`);
            }
        }
    }
}
