 ...
 */
const uploadStatementFile = async (fileData) => {
 ...
 * @returns {Promise<StatementFile>}
 */
const updateStatementFileStatus = async (statementFileId, status) => {
 ...
 */
const getUploadStatus = async (statementFileId) => {
 ...
 */
const parseCSVStatement = (content) => {
 ...
 * @returns {StatementParseResult}
 */
const parsePDFStatement = (buffer) => {
    console.log({ buffer });
 ...
 */
const parseStatement = async (statementFileId) => {
    ...
    try {
        ...
        const dbParseResult = await prisma.parseResult.create({
            data: {
                ...
                accounts: parseResult.accounts, // Stored as JSON
        ...
        throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, `Failed to parse statement: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
};
/**
 ...
 */
const queryStatementFiles = async (filter, options, keys = ['id', 'filename', 'type', 'size', 'uploadedAt', 'status', 'clientId']) => {
 ...
 */
const getStatementFileById = async (id, keys = ['id', 'filename', 'type', 'size', 'uploadedAt', 'status', 'clientId']) => {
    return (await prisma.statementFile.findUnique({
        where: { id },
        select: keys.reduce((obj, k) => ({ ...obj, [k]: true }), {})
    }));
 ...
 */
const getStatementFileWithParseResult = async (id) => {
    return await prisma.statementFile.findUnique({
        where: { id },
        include: { ParseResult: true }
    });
 ...
 * @returns {Promise<StatementFile>}
 */
const deleteStatementFileById = async (statementFileId) => {
    const statementFile = await getStatementFileById(statementFileId);
    if (!statementFile) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Statement file not found');
    }
 ...
 */
const generateUploadSignedUrl = async (statementFileId) => {
    const statementFile = await getStatementFileById(statementFileId);
    if (!statementFile) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Statement file not found');
    }
 ...
 */
const uploadStatementFiles = async (uploadData) => {
    // Validate client exists
    const client = await prisma.client.findUnique({
    ...
    // Process each file
    const results = await Promise.all(uploadData.files.map(async (fileData) => {
        try {
            // Validate file type
            const allowedTypes = [
                'application/pdf',
 ...
 */
const parseStatementFiles = async (statementFileIds) => {
    ...
    // Parse each file
    const results = await Promise.all(statementFiles.map(async (statementFile) => {
        try {
