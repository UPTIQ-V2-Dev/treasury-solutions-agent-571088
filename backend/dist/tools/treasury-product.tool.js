...
const getTreasuryProductsTool = {
    ...
    fn: async (inputs) => {
        ...
        if (inputs.isActive !== undefined)
            filter.isActive = inputs.isActive;
        if (inputs.name)
            filter.name = { contains: inputs.name, mode: 'insensitive' };
        const options = {
...
const getTreasuryProductsByCategoryTool = {
...
const getActiveTreasuryProductsTool = {
