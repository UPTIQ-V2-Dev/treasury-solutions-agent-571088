import treasuryProductController from '../controllers/treasury-product.controller.ts';
...
vi.mock('../utils/catchAsyncWithAuth.ts', () => ({
    default: (fn: Function) => (req: any, res: any, next: any) => fn(req, res, next)
}));

const mockTreasuryProductService = treasuryProductService as any;
const mockPick = pick as any;
...
describe('Treasury Product Controller', () => {
    ...
    beforeEach(() => {
        ...
        mockPick.mockImplementation((obj: any, keys: string[]) => {
            const result: any = {};
    ...
    describe('getTreasuryProducts', () => {
        ...
        it('should get treasury products successfully', async () => {
            ...
            expect(mockTreasuryProductService.queryTreasuryProducts).toHaveBeenCalledWith(
                { category: 'sweep', isActive: true },
                { sortBy: 'name', limit: 10, page: 1 },
                expect.any(Array)
            );
    ...
    describe('getTreasuryProduct', () => {
        ...
        it('should throw ApiError if product not found', async () => {
            ...
            await expect(
                treasuryProductController.getTreasuryProduct(mockRequest, mockResponse, mockNext)
            ).rejects.toThrow(new ApiError(httpStatus.NOT_FOUND, 'Treasury product not found'));

            expect(mockTreasuryProductService.getTreasuryProductById).toHaveBeenCalledWith(
                'nonexistent-product',
                expect.any(Array)
            );
