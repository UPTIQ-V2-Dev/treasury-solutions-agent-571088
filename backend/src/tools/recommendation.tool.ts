...
const getRecommendationsTool: MCPTool = {
    ...
    fn: async (inputs: {
        ...
        const filter: any = {};
        if (inputs.analysisId) filter.analysisId = inputs.analysisId;
        if (inputs.status) filter.status = inputs.status;
        if (inputs.priority) filter.priority = inputs.priority;
        if (inputs.productId) filter.productId = inputs.productId;
...
const getRecommendationTool: MCPTool = {
...
const getRecommendationsByAnalysisTool: MCPTool = {
    ...
    fn: async (inputs: {
        ...
        const filter: any = { analysisId: inputs.analysisId };
        if (inputs.status) filter.status = inputs.status;
        if (inputs.priority) filter.priority = inputs.priority;
...
const getRecommendationsByPriorityTool: MCPTool = {
    ...
    fn: async (inputs: {
        ...
        const filter: any = { priority: inputs.priority };
        if (inputs.status) filter.status = inputs.status;
        if (inputs.analysisId) filter.analysisId = inputs.analysisId;
...
const getRecommendationsByStatusTool: MCPTool = {
    ...
    fn: async (inputs: {
        ...
        const filter: any = { status: inputs.status };
        if (inputs.analysisId) filter.analysisId = inputs.analysisId;
        if (inputs.priority) filter.priority = inputs.priority;
