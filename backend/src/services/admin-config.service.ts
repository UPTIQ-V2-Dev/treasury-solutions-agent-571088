...
import { Prisma, SystemConfig } from '../generated/prisma/index.js';
...
const getConfiguration = async (): Promise<{
    ...
    Object.keys(DEFAULT_CONFIG).forEach(category => {
        Object.keys((DEFAULT_CONFIG as any)[category]).forEach(key => {
            if (config[category][key] === undefined) {
                config[category][key] = (DEFAULT_CONFIG as any)[category][key];
            }
        });
    });
...
const updateConfiguration = async (
    ...
    Object.keys(updateData).forEach(category => {
        const categoryData = updateData[category as keyof typeof updateData];
 ...
 */
const initializeDefaultConfiguration = async (updatedBy: string): Promise<void> => {
    const configEntries: Prisma.SystemConfigCreateInput[] = [];

    Object.keys(DEFAULT_CONFIG).forEach(category => {
        Object.keys((DEFAULT_CONFIG as any)[category]).forEach(key => {
            const configKey = `${category}.${key}`;
            const configValue = (DEFAULT_CONFIG as any)[category][key];
...
const validateConfiguration = (config: any): void => {
    if (config.thresholds) {
        ...
        if (
            thresholds.idleCashThreshold !== undefined &&
        ...
        if (
            thresholds.liquidityWarningThreshold !== undefined &&
        ...
        if (
            thresholds.lowBalanceThreshold !== undefined &&
        ...
        if (
            thresholds.highRiskThreshold !== undefined &&
    ...
    if (config.integrations) {
        ...
        if (
            integrations.apiRateLimit !== undefined &&
        ...
        if (
            integrations.maxFileSize !== undefined &&
    ...
    if (config.security) {
        ...
        if (
            security.passwordExpiry !== undefined &&
        ...
        if (
            security.auditLogRetention !== undefined &&
 ...
 */
const getConfigurationByKey = async (key: string): Promise<any> => {
    ...
    if (!config) {
        ...
        if ((DEFAULT_CONFIG as any)[category] && (DEFAULT_CONFIG as any)[category][subKey] !== undefined) {
            return (DEFAULT_CONFIG as any)[category][subKey];
        }
